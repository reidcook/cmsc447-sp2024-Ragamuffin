class leaderboard2 extends Phaser.Scene {
    constructor() {
        super("leaderboard2");
    }
    init(data){
    ...
    async create() {
        ...
        try {
            const response = await fetch('http://localhost:3000/leaderboard/level2');
            if (!response.ok) {
            ...
            for (let i = 0; i < data.length; i++) {
                const { username, level_2_score } = data[i];
                this.add.text(200, 100 + i * 30, `${i + 1}. ${username}: ${level_2_score}`, { fontSize: '24px', fill: '#0f0' });
            }
        } catch (error) {
            console.error('Error fetching level 2 leaderboard data:', error.message);
