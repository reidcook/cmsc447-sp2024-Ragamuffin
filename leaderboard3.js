class leaderboard3 extends Phaser.Scene {
    constructor() {
        super("leaderboard3");
    }
    init(data){
    ...
    async create() {
        ...
        try {
            const response = await fetch('http://localhost:3000/leaderboard/level3');
            if (!response.ok) {
            ...
            restartButton.on('pointerdown', () => {this.scene.start("level3", {color: this.color});});
            levelButton.setInteractive();
            levelButton.on('pointerdown', () => {this.scene.start("levelselect", {color: this.color});});
            this.add.text(150, 50, 'Top Level 3 Scores', { fontSize: '32px', fill: '#0f0' });
            for (let i = 0; i < data.length; i++) {
                const { username, level_3_score } = data[i];
                this.add.text(200, 100 + i * 30, `${i + 1}. ${username}: ${level_3_score}`, { fontSize: '24px', fill: '#0f0' });
            }
        } catch (error) {
            console.error('Error fetching level 3 leaderboard data:', error.message);
